...
class NoSQLClient:
    ...
    def update_cloudant_document(
        self, db, doc_id, doc_new: dict, dict_of_list_values: dict = {}, attachments=[]
    ):
        """
        ...
        try:
            ...
            for k, v in doc_new.items():
                if k in list(
                    dict_of_list_values.keys()
                ):  # if we have the attributes list to modify
                    attr = []
                    for i in range(
                        len(v)
                    ):  # Range the interval of the list of the attribut
                        elt = v[i].copy()
                        new_elt = v[i].copy()

                        if (
                            k == "attachments" and attachments
                        ):  # if we have the attachments to modify
                            for elt_attach in attachments:  # Go through the attachments
                                if elt_attach.get("name") and elt_attach.get(
                                    "name"
                                ) == elt.get("name"):
                                    elt = attachments[i]
                        ...
                        for _v in dict_of_list_values[
                            k
                        ]:  # Go through the attributs list of the doc attr than we going modify
                            if elt.get(_v):
